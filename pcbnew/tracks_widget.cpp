...
/*  This source module implements the track length panel widget
    @todo everything
*/
...
int TRACK_WIDGET::encodeId( int aColumn, int aId )
{
...
LAYER_NUM TRACK_WIDGET::getDecodedId( int aControlId )
{
...
void TRACK_WIDGET::OnLeftDownLayers( wxMouseEvent& event )
{
    int row;
    LAYER_NUM layer;
...
void TRACK_WIDGET::OnLayerCheckBox( wxCommandEvent& event )
{
...
void TRACK_WIDGET::OnRenderSwatchChanged( wxCommandEvent& aEvent )
{
    auto eventSource = static_cast<COLOR_SWATCH*>( aEvent.GetEventObject() );
...
void TRACK_WIDGET::OnRenderCheckBox( wxCommandEvent& event )
{
    wxCheckBox* eventSource = (wxCheckBox*) event.GetEventObject();
    LAYER_NUM id = getDecodedId( eventSource->GetId() );
...
void TRACK_WIDGET::OnTabChange( wxNotebookEvent& event )
{
...
wxWindow* TRACK_WIDGET::getLayerComp( int aRow, int aColumn ) const
{
...
int TRACK_WIDGET::findLayerRow( LAYER_NUM aLayer ) const
{
...
wxWindow* TRACK_WIDGET::getRenderComp( int aRow, int aColumn ) const
{
...
int TRACK_WIDGET::findRenderRow( int aId ) const
{
...
void TRACK_WIDGET::insertLayerRow( int aRow, const ROW& aSpec )
{
    wxASSERT( aRow >= 0 );
...
void TRACK_WIDGET::insertRenderRow( int aRow, const ROW& aSpec )
{
    wxASSERT( aRow >= 0 );
...
wxSize TRACK_WIDGET::GetBestSize() const
{
...
int TRACK_WIDGET::GetLayerRowCount() const
{
...
int TRACK_WIDGET::GetRenderRowCount() const
{
...
void TRACK_WIDGET::AppendLayerRow( const ROW& aRow )
{
...
void TRACK_WIDGET::ClearLayerRows()
{
    m_LayersFlexGridSizer->Clear( true );
}


void TRACK_WIDGET::AppendRenderRow( const ROW& aRow )
{
...
void TRACK_WIDGET::ClearRenderRows()
{
    m_RenderFlexGridSizer->Clear( true );
}


void TRACK_WIDGET::SelectLayerRow( int aRow )
{
...
void TRACK_WIDGET::SelectLayer( LAYER_NUM aLayer )
{
...
LAYER_NUM TRACK_WIDGET::GetSelectedLayer()
{
...
void TRACK_WIDGET::SetLayerVisible( LAYER_NUM aLayer, bool isVisible )
{
    int row = findLayerRow( aLayer );
    if( row >= 0 )
    {
...
bool TRACK_WIDGET::IsLayerVisible( LAYER_NUM aLayer )
{
    int row = findLayerRow( aLayer );
    if( row >= 0 )
    {
...
void TRACK_WIDGET::SetLayerColor( LAYER_NUM aLayer, COLOR4D aColor )
{
    int row = findLayerRow( aLayer );
    if( row >= 0 )
    {
...
COLOR4D TRACK_WIDGET::GetLayerColor( LAYER_NUM aLayer ) const
{
    int row = findLayerRow( aLayer );
    if( row >= 0 )
    {
...
void TRACK_WIDGET::SetRenderState( int aId, bool isSet )
{
...
bool TRACK_WIDGET::GetRenderState( int aId )
{
    int row = findRenderRow( aId );
...
void TRACK_WIDGET::UpdateLayouts()
{
...
void TRACK_WIDGET::UpdateLayerIcons()
{
    int rowCount = GetLayerRowCount();
...
/**
 * Class MYFRAME
 * is a test class here to exercise the TRACK_WIDGET and explore use cases.
...
class MYFRAME : public wxFrame
{
    ...
    // abstract methods.
    class MYLAYERS : public TRACK_WIDGET
    {
    public:
...
public:
            ...
            wxSize( 800, 600 ), wxDEFAULT_FRAME_STYLE )
    {
        ...
        // add some layer rows
        static const TRACK_WIDGET::ROW layerRows[] = {
            TRACK_WIDGET::ROW( wxT("layer 1"), 0, RED, wxT("RED"), false ),
            TRACK_WIDGET::ROW( wxT("layer 2"), 1, GREEN, wxT("GREEN"), true ),
            TRACK_WIDGET::ROW( wxT("brown_layer"), 2, BROWN, wxT("BROWN"), true ),
            TRACK_WIDGET::ROW( wxT("layer_4_you"), 3, BLUE, wxT("BLUE"), false ),
        };
