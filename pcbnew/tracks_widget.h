...
/**
 * Class TRACK_WIDGET
 * is abstract and is used to manage a list of tracks with net classes. You must derive 
 * from it to use it so you can implement the abstract functions which recieve the
 * events.  Each track is given a net class, the net class can be changed within the UI
 * provided in the dialog design rules.  This widget knows nothing of the client code, meaning
 * it has no knowledge of a BOARD or anything.  To use it you must derive from
 * this class and implement the abstract functions:
 * <p> void OnTrackMeasure( int track, bool measureTrack );
 *
 */
class TRACK_WIDGET : public wxPanel
{
public:
    /**
     * Struct ROW
     * provides all the data needed to add a row to a TRACK_WIDGET.  This is
     * part of the public API for a TRACK_WIDGET.
     */
    struct ROW
    {
...
public:
     ...
     * every operation.
     */
    TRACK_WIDGET( wxWindow* aParent, wxWindow* aFocusOwner, int aPointSize = -1,
